...
export class RecoveryProviderImpl implements RecoveryProvider {
  ...
  async generateRecoveryKey(): Promise<string> {
    const rawBytes = this.provider.randomBytes(KEY_LENGTH);
  ...
  async createPackage(
    ...
    const result = await this.provider.encrypt(recoverySecret, vaultRootKey, aad);
  ...
  async verifyRecoveryKey(key: string): Promise<boolean> {
    ...
    try {
      ...
      const digestOk = await verifyEnvelopeDigest(this.storedPackage.envelope);
