...
|---|---|---|---|
| expo-sqlite | 57.0.3 (pinned in lockfile) | SQLCipher-encrypted local DB | `npx expo install expo-sqlite` |
| expo-secure-store | 57.0.4 (pinned in lockfile) | OS Keychain/Keystore for bootstrap secret | `npx expo install expo-secure-store` |
| expo-crypto | 57.0.3 (pinned in lockfile) | Native AES-256-GCM, secure random, SHA-256 | `npx expo install expo-crypto` |
...
| Deterministic known-answer vector (fixed key/nonce) | PASS | Vitest |
...
**Total: 38 vault-native tests + 23 existing tests = 61 tests, all passing.**
