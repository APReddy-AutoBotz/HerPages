...
export async function buildEnvelope(
  ...
  const ciphertextSha256 = bytesToHex(await provider.sha256(ciphertextWithTag));
 ...
 */
export async function verifyEnvelopeDigest(envelope: VaultEnvelope, provider: AeadProvider): Promise<boolean> {
  const ciphertextWithTag = base64ToBytes(envelope.ciphertext_b64);
  const computed = bytesToHex(await provider.sha256(ciphertextWithTag));
